...
#### 哈希路由
- [ ] **功能**: 根据键的哈希值路由到对应的 Raft 组
- [ ] **算法**: `shard_id = hash(key) % shard_count`
- [ ] **实现**: 
  - 一致性哈希或简单取模
  - 路由表缓存（避免重复计算）
- [ ] **验证**: 
...
- [ ] **步骤**:
  1. **数据合并**：
     ...
     - 合并到目标组（处理键冲突）
  2. **配置变更**：
     - 使用 Joint Consensus 合并配置
     - 移除源组
  3. **路由更新**：
     - 更新路由表（多个 shard_id → 一个 shard_id）
  4. **清理**：
     - 删除源组数据
- [ ] **键冲突处理**: 定义合并策略（保留最新/最大版本等）
- [ ] **验证**: 
  - 合并后数据完整性
  - 合并后所有键可访问
...
- [ ] **功能**: 多个 Raft 组并发处理请求
- [ ] **实现**: 
  - 使用 MultiRaftDriver 管理多个 RaftState
  - 不同组的事件并发处理
- [ ] **验证**: 
  - 不同组的请求互不阻塞
  - 吞吐量线性扩展（组数 × 单组吞吐）
...
#### KVStateMachine（每个 Raft 组独立）
- [ ] **结构**: 内存 HashMap 存储键值对（每个组独立）
- [ ] **Apply**: 将 Raft 日志应用到状态机
- [ ] **快照**: 
  - 创建快照（序列化该组的所有键值对）
  - 恢复快照（反序列化并重建该组状态）
- [ ] **线程安全**: 使用 RwLock 保护并发访问
- [ ] **多实例**: 每个 Raft 组有独立的状态机实例
- [ ] **验证**:
  - 状态机状态一致性（每个组内部）
  - 快照创建和恢复（每个组独立）
  - 多组数据隔离
