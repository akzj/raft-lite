...
mod tests {
    ...
    fn create_test_request_vote() -> RequestVoteRequest {
        RequestVoteRequest {
            term: 1,
    ...
    fn create_test_append_entries() -> AppendEntriesRequest {
        AppendEntriesRequest {
            term: 1,
    ...
    async fn test_basic_message_delivery() {
        ...
        let result = network1.send_request_vote_request(node1.clone(), node2.clone(), vote_req.clone()).await;
        assert!(result.is_ok());
    ...
    async fn test_append_entries_message_delivery() {
        ...
        let result = network_leader.send_append_entries_request(leader.clone(), follower.clone(), append_req.clone()).await;
        assert!(result.is_ok());
    ...
    async fn test_bidirectional_communication() {
        ...
        // Node1 -> Node2
        let vote_req = create_test_request_vote();
        network1.send_request_vote_request(node1.clone(), node2.clone(), vote_req).await.unwrap();
        
        // Node2 -> Node1  
        let append_req = create_test_append_entries();
        network2.send_append_entries_request(node2.clone(), node1.clone(), append_req).await.unwrap();
        ...
        assert!(msg1.is_ok());
        assert!(msg2.is_ok());
        assert!(matches!(msg1.unwrap().unwrap(), NetworkEvent::RequestVote(_)));
        assert!(matches!(msg2.unwrap().unwrap(), NetworkEvent::AppendEntriesRequest(_)));
    }
    ...
    async fn test_message_ordering_fifo() {
        ...
        for i in 1..=5 {
            ...
            network_sender.send_request_vote_request(sender.clone(), receiver.clone(), vote_req).await.unwrap();
        }
    ...
    async fn test_latency_simulation() {
        ...
        let vote_req = create_test_request_vote();
        network.send_request_vote_request(sender, receiver, vote_req).await.unwrap();
    ...
    async fn test_batch_processing() {
        ...
        for i in 1..=5 {
            let mut vote_req = create_test_request_vote();
            vote_req.term = i;
            network.send_request_vote_request(sender.clone(), receiver.clone(), vote_req).await.unwrap();
        }
    ...
    async fn test_config_update() {
        ...
        assert_eq!(updated_config.batch_max_wait_ms, new_config.batch_max_wait_ms);
    }
    ...
    async fn test_nonexistent_target_node() {
        ...
        let vote_req = create_test_request_vote();
        let result = network.send_request_vote_request(sender, nonexistent, vote_req).await;
        assert!(result.is_ok());
    ...
    async fn test_all_message_types() {
        ...
        network.send_request_vote_response(sender.clone(), receiver.clone(), vote_resp).await.unwrap();
        
        let append_req = create_test_append_entries();
        network.send_append_entries_request(sender.clone(), receiver.clone(), append_req).await.unwrap();
        ...
        network.send_append_entries_response(sender.clone(), receiver.clone(), append_resp).await.unwrap();
        
        use crate::{Snapshot, ClusterConfig};
        let _snapshot = Snapshot {
            index: 1,
            term: 1,
            data: vec![1, 2, 3],
            config: ClusterConfig::simple(
                vec![sender.clone()].into_iter().collect(),
                0
            ),
        };
        ...
        network.send_install_snapshot_request(sender.clone(), receiver.clone(), install_req).await.unwrap();
        ...
        network.send_install_snapshot_response(sender, receiver, install_resp).await.unwrap();
    ...
    async fn test_dispatch_network_event() {
        ...
        assert!(matches!(raft_event, Some(Event::AppendEntriesResponse(_, _))));
    }
    ...
    async fn test_concurrent_message_sending() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                let mut vote_req = create_test_request_vote();
                vote_req.term = i + 1;
                network_clone.send_request_vote_request(sender_clone, receiver_clone, vote_req).await
            });
            handles.push(handle);
        }
    ...
    async fn test_message_drop_simulation() {
        ...
        let vote_req = create_test_request_vote();
        let result = network.send_request_vote_request(sender, receiver, vote_req).await;
