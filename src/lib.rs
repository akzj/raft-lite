...
impl RaftState {
    ...
    async fn broadcast_append_entries(&mut self) {
        ...
        for peer in peers {
            ...
            // 使用反馈控制的批次大小
            let batch_size = self.get_adaptive_batch_size(&peer);
            ...
            // 记录InFlight请求
            self.track_inflight_request(&peer, request_id, next_idx + entries_len as u64, now);
    ...
    async fn handle_append_entries_response(
        ...
        if response.term == self.current_term {
            ...
            // 记录反馈（成功或失败）
            self.record_append_entries_response_feedback(&peer, response.request_id, response.success);
            ...
            if entry.is_config {
                if let Ok(new_config) = serde_json::from_slice::<ClusterConfig>(&entry.command) {
                    ...
                    if self.config.log_index() > new_config.log_index() {
                        ...
                        // 检查 learner 删除
                        let old_learners =
                            old_config.learners.as_ref().cloned().unwrap_or_default();
                        let new_learners =
                            new_config.learners.as_ref().cloned().unwrap_or_default();
    ...
    fn can_send_to_peer(&self, peer: &RaftId) -> bool {
    ...
    /// 获取基于反馈控制的批次大小
    fn get_adaptive_batch_size(&mut self, peer: &RaftId) -> u64 {
        // 如果是第一次发送，使用初始批次大小
    ...
    fn track_inflight_request(&mut self, peer: &RaftId, request_id: RequestId, next_ack: u64, _send_time: Instant) {
        // 使用新的高效方法
    ...
    fn remove_inflight_request(&mut self, peer: &RaftId, request_id: RequestId) {
        // 使用新的高效方法
        if self.remove_inflight_request_with_timeout(peer, request_id) {
            let remaining_count = self.inflight_requests
                .get(peer)
                .map(|reqs| reqs.len())
                .unwrap_or(0);
    ...
    /// 记录成功响应的反馈
    fn record_success_response(&mut self, peer: &RaftId, request_id: RequestId) {
        let removed_request = self.inflight_requests
            .get_mut(peer)
    ...
    /// 记录AppendEntries响应反馈
    fn record_append_entries_response_feedback(&mut self, peer: &RaftId, request_id: RequestId, success: bool) {
        if success {
    ...
    fn record_response_feedback(&mut self, peer: &RaftId, response_time: Duration, success: bool) {
        // 记录响应时间
        let response_times = self.response_times.entry(peer.clone()).or_insert_with(VecDeque::new);
        ...
        // 记录成功率
        let success_rates = self.success_rates.entry(peer.clone()).or_insert_with(VecDeque::new);
            ...
            if let Some(timeout_queue) = self.inflight_timeout_queue.get_mut(&peer) {
                ...
                while let Some((_request_id, send_time)) = timeout_queue.front() {
                    if now.duration_since(*send_time) > timeout_duration {
                        let (expired_request_id, expired_send_time) = timeout_queue.pop_front().unwrap();
                        expired_items.push((peer.clone(), expired_request_id, expired_send_time, timeout_duration));
                    } else {
    ...
    fn calculate_smart_timeout(&self, peer: &RaftId) -> Duration {
        ...
        debug!(
            ...
            timeout, 
            avg_response_time, 
            self.options.timeout_response_factor
        );
    ...
    fn track_inflight_request_with_timeout(&mut self, peer: &RaftId, request_id: RequestId, next_ack_index: u64) {
        let now = Instant::now();
    ...
    /// 移除InFlight请求 - 简单且高效
    fn remove_inflight_request_with_timeout(&mut self, peer: &RaftId, request_id: RequestId) -> bool {
        // 从inflight_requests移除
        let removed = self.inflight_requests
            .get_mut(peer)
            .and_then(|peer_requests| peer_requests.remove(&request_id))
            .is_some();
    ...
    /// 优化的周期性超时检查 - 在心跳时调用
    fn periodic_timeout_check(&mut self) {
