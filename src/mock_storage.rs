...
use crate::{RaftId, Storage, StorageResult};
...
mod tests {
    ...
    fn create_test_config(voters: Vec<(&str, &str)>) -> ClusterConfig {
        let voter_set: HashSet<RaftId> = voters.into_iter()
    ...
    fn create_test_snapshot(index: u64, term: u64, data: &str) -> Snapshot {
        Snapshot {
            ...
            config: create_test_config(vec![("group1", "node1"), ("group1", "node2"), ("group1", "node3")]),
        }
    }
    ...
    async fn test_log_entry_operations() {
        ...
        storage.append_log_entries(node_id.clone(), &entries).await.unwrap();
        ...
        // 测试获取日志条目范围
        let retrieved = storage.get_log_entries(node_id.clone(), 1, 3).await.unwrap();
        assert_eq!(retrieved.len(), 2);
        assert_eq!(retrieved[0].index, 1);
        assert_eq!(retrieved[1].index, 2);

        // 测试获取全部日志
        let all_entries = storage.get_log_entries(node_id.clone(), 1, 4).await.unwrap();
    ...
    async fn test_truncate_log_suffix() {
        ...
        // 截断索引3及之后的日志
        storage.truncate_log_suffix(node_id.clone(), 3).await.unwrap();

        let remaining = storage.get_log_entries(node_id.clone(), 1, 10).await.unwrap();
        ...
        // 测试截断不存在的索引
        storage.truncate_log_suffix(node_id.clone(), 10).await.unwrap();
    ...
    async fn test_truncate_log_prefix() {
        ...
        // 截断索引3之前的日志
        storage.truncate_log_prefix(node_id.clone(), 3).await.unwrap();
        ...
        // 测试截断所有日志
        storage.truncate_log_prefix(node_id.clone(), 5).await.unwrap();
        let empty = storage.get_log_entries(node_id.clone(), 1, 10).await.unwrap();
        assert_eq!(empty.len(), 0);
    ...
    async fn test_snapshot_operations() {
        ...
        storage.save_snapshot(node_id.clone(), snapshot.clone()).await.unwrap();
        ...
        storage.save_snapshot(node_id.clone(), new_snapshot.clone()).await.unwrap();
    ...
    async fn test_cluster_config_operations() {
        ...
        // 测试保存和加载集群配置
        let config = create_test_config(vec![("group1", "node1"), ("group1", "node2"), ("group1", "node3")]);
        storage.save_cluster_config(node_id.clone(), config.clone()).await.unwrap();

        let loaded = storage.load_cluster_config(node_id.clone()).await.unwrap();
        assert_eq!(loaded, config);

        // 测试更新配置
        let new_config = create_test_config(vec![("group1", "node1"), ("group1", "node2"), ("group1", "node3"), ("group1", "node4")]);
        storage.save_cluster_config(node_id.clone(), new_config.clone()).await.unwrap();
    ...
    async fn test_concurrent_operations() {
        ...
        for i in 1..=10 {
            ...
            let handle = task::spawn(async move {
                ...
                storage_clone.append_log_entries(node_id_clone, &[entry]).await.unwrap();
            });
            handles.push(handle);
        }
        ...
        // 验证所有日志都被正确写入
        let all_entries = storage.get_log_entries(node_id.clone(), 1, 11).await.unwrap();
    ...
    async fn test_edge_cases() {
        ...
        // 测试相等的low和high
        let same_range = storage.get_log_entries(node_id.clone(), 2, 2).await.unwrap();
        assert_eq!(same_range.len(), 0);

        // 测试超出范围的查询
        let out_of_range = storage.get_log_entries(node_id.clone(), 10, 20).await.unwrap();
        assert_eq!(out_of_range.len(), 0);

        // 测试倒序范围（high < low）
        let reverse_range = storage.get_log_entries(node_id.clone(), 3, 1).await.unwrap();
        assert_eq!(reverse_range.len(), 0);

        // 测试截断不存在的索引
        storage.truncate_log_suffix(node_id.clone(), 100).await.unwrap();
        let unchanged = storage.get_log_entries(node_id.clone(), 1, 10).await.unwrap();
    ...
    async fn test_multiple_nodes() {
        ...
        storage.save_hard_state(node1.clone(), 5, Some(candidate1)).await.unwrap();
        storage.save_hard_state(node2.clone(), 3, Some(candidate2.clone())).await.unwrap();
    ...
    async fn test_append_multiple_batches() {
        ...
        storage.append_log_entries(node_id.clone(), &batch1).await.unwrap();

        let batch2 = vec![
            create_test_log_entry(3, 2, "command3"),
            create_test_log_entry(4, 2, "command4"),
        ];
        storage.append_log_entries(node_id.clone(), &batch2).await.unwrap();

        // 验证所有日志都存在
        let all = storage.get_log_entries(node_id.clone(), 1, 5).await.unwrap();
        assert_eq!(all.len(), 4);
